...
GoF 디자인 패턴 4종을 적용해 확장성과 신뢰성을 확보했습니다.

### 핵심 공학 포인트 — Factory 패턴

> **"벌금 유형이 늘어나도 정산 로직을 건드리지 않는다"**

결석·지각 등 벌금 유형마다 별도의 정산 객체(`AbsencePenalty`, `LatePenalty`)를 `PenaltyFactory`가 생성합니다.  
`FineCalculatorService`는 `Penalty` 인터페이스만 바라보기 때문에, 새로운 벌금 유형(예: 과제 미제출)이 생겨도 **기존 코드를 수정하지 않고** 구현체 하나만 추가하면 됩니다. (OCP)

```
PenaltyFactory.create(ABSENCE) → AbsencePenalty  →  결석 × 5,000원
PenaltyFactory.create(LATE)    → LatePenalty     →  지각 × 2,000원
PenaltyFactory.create(???)     → ???Penalty      →  추가 유형 확장 가능
```
